...
if selected_switch = "Prediction of demand":
    ...
    with st.container():
        ...
        if model == "Compare models":
                left_column, middle_column = st.columns([2,3])
                left_column_hidden, right_column = st.columns([2,3])
        else:
...
if selected_switch == "Dashboard demand":
    ...
    detail_stations = st.sidebar.selectbox('Bottom information', ["Evolution demand", "Stations information"])
    ...
    with st.container():
        if detail_stations == "Stations information":
